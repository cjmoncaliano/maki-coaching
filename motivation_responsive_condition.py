...
class GameState(object):
	...
	self.waiting_list=['a','b','c','d','e'] # phrases to say when waiting for answer
	...
	condition_argPath = "condition4/"
	question_argPath = "question/a.wav"
	wait_argPath = "wait/"
	ending_argPath = "ending/a.wav"
	...
	while True:
		...
		if self.request_response == 1:
			...
			play_file=self.waiting_list[ran_num]
			self.speak(wait_argPath+play_file+".wav")
			while self.request_response != 2:
				pass
			self.speak(question_argPath)
			start = time.time()
		elif self.request_response ==  9:
			#movy.blinking()
			self.speak(ending_argPath)
			break
		elif ((time.time() - start >= 100) and (time.time() - start <= 102)):
