...
class GameState(object):
	...
	condition_argPath = "condition1/"
	#while self.request_response != 7:
	#	pass			  # 7 to start the task
	...
	while True:
		...
		if self.request_response==1:  ## js press 1 once when the kid has stopped talking
			movy.noding()
			rospy.sleep(2)
			self.speak(condition_argPath + "b.wav")
			self.speak(condition_argPath + "c.wav")
			#start = time.time()
